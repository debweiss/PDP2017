...
class NeighborhoodCandyExpression {
    ...
    /** Expression that describes what has to be true about a candy to know that it exists in the neighborhood.
     * In this case, it needs to either belong to a mansion, a duplex, a detached house, or townhome.
    ...
    private IExpression getNeighborhoodCandyExpression() {
        ...
        // the candy belongs to the neighborhood if it belongs to either a mansion, duplex, detached house, or townhome
    ...
    /** Expression that describes what has to be true about a candy to know that it belongs in a mansion.
     * @return an expression that is used to evaluate incoming candies.
     */
    @NotNull
    private IExpression getMansionCandyExpression() {

        // the candy belongs to a mansion if it is any of the sizes and candy names that mansions have
        return new OrExp(new OrExp(getMansionFunSizeCandyExpression(), getMansionKingSizeCandyExpression()),
                            getMansionSuperSizeCandyExpression());
    }
    ...
    /** Expression that describes what has to be true about a candy to know that it belongs in a duplex.
     * @return an expression that is used to evaluate incoming candies.
     */
    @NotNull
    private IExpression getDuplexCandyExpression() {

        // the candy belongs to a duplex if it is any of the sizes and candy names that mansions have
        return new OrExp(new OrExp(getDuplexFunSizeCandyExpression(), getDuplexKingSizeCandyExpression()),
           getDuplexSuperSizeCandyExpression());
    }
    ...
    private IExpression getDuplexFunSizeCandyExpression() {
        ...
        // A Duplex's fun size candies are any of kit kat, whoppers, milky way, crunch
        return new AndExp(funSize, (new OrExp(new OrExp(kitKat, whoppers), new OrExp(milkyWay, crunch))));
    }
    ...
    private IExpression getDuplexSuperSizeCandyExpression() {
        ...
        // A Duplex's super size candies are any of toblerone, baby ruth, or almond joy
        return new AndExp(superSize, (new OrExp(new OrExp(toblerone, babyRuth), almondJoy)));
    }

    /************************** DetachedHouse-related expressions*************************************/

    /** Expression that describes what has to be true about a candy to know that it belongs in a detached house.
     * @return an expression that is used to evaluate incoming candies.
     */
    @NotNull
    private IExpression getDetachedHouseCandyExpression() {

        // the candy belongs to a detached house if it is any of the sizes and candy names that detached houses have
        return new OrExp(new OrExp(getDetachedHouseFunSizeCandyExpression(), getDetachedHouseKingSizeCandyExpression()),
                            getDetachedHouseSuperSizeCandyExpression());
    }
    ...
    private IExpression getDetachedHouseSuperSizeCandyExpression() {
        ...
        // A Detached House's super size candies are any of kit kat, whoppers, milky way, crunch
        return new AndExp(superSize, (new OrExp(new OrExp(kitKat, whoppers), new OrExp(milkyWay, crunch))));
    }

    /************************** Townhome-related expressions*************************************/

    /** Expression that describes what has to be true about a candy to know that it belongs in a townhome.
     * @return an expression that is used to evaluate incoming candies.
     */
    @NotNull
    private IExpression getTownHomeCandyExpression() {

        // A Townhome only has regular size candies, and they are kit kat, whoppers, twix, snickers, mars
        // baby ruth, almond joy, and toblerone
        return new AndExp(regularSize, (new OrExp(new OrExp(new OrExp(kitKat, whoppers), new OrExp(twix, snickers))
           , new OrExp(new OrExp(mars, babyRuth), new OrExp(almondJoy, toblerone)))));
