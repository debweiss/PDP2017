...
class CandyParser {
     ...
     * @return true if the filename matches (is valid), returns false if the
    ...
    /**
     * Parses a string that contains candy name and size and returns
     * an array list with two strings, one with candy name and one with
     * candy size.
     *
     * @param candy string containing the name and size of the candy
     *              (one candy from the list of candies)
    ...
    List<String> parseCandy(String candy) {
        ...
        // If the string has the word "size" in it
        if (candy.toLowerCase().contains("size")) {
            
            // take every character up to end of 'Size' to get candy size
            candy = candy.trim();
            candySize = candy.substring(0, candy.indexOf(" ") + 1) + "size";
            candySize = candySize.trim();
            
            /* take every character from the end of 'Size' to the end of
            the candy name for candyName */
            candyName = (candy.substring(candySize.length() + 1,
                candy.length())).trim();
            
        } else {
            
            // if a size isn't listed in the doc, call it regular size
            candySize = CandyCategories.SizeCategory.REGULAR_SIZE
                .getValue().trim();
        ...
        candyNameAndSize.add(candySize); // add candy size to array
